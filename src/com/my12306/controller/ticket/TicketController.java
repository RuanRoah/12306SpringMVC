...
public class TicketController {
	@RequestMapping("queryticket.do")
	public ModelAndView queryTicket(@RequestParam("start") String start, @RequestParam("end") String end,
			@RequestParam("dates") String date,HttpServletRequest request,@RequestParam("seat") String seat,HttpSession session) {
		// start 出发点 end 终点 date 日期
		System.out.println(start+end+date+seat);
		...
		for (int i = 0; i < queryTrains.size(); i++) {
				...
				List<Seat> queryseats=ticketService.querySeats(queryTrains.get(i).TRAINID,seat);
		...
		session.setAttribute("userpeoples",up);
